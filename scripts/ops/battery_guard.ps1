# Hibernate this laptop when the battery reaches 16% while discharging.
# Owner's instruction 2026-09-05: "work till the battery reaches 15%, pause,
# then hibernate." Raised to 16% on the owner's instruction 2026-09-09 ("at 16
# percent battery hibernate the laptop"), given while the assistant was to keep
# working unattended overnight -- one percent of extra margin matters more when
# nobody is watching the number.
#
...
while ($true) {
    ...
    if ($b) {
        ...
        if ($discharging -and $pct -le 16) {
            ...
            # if the laptop wakes still discharging at <=16%, the next loop
        ...
        if ($pct -le 20 -and $discharging) { Write-Output "BATTERY LOW $pct% (discharging) -- hibernate at 16%" }
    }
