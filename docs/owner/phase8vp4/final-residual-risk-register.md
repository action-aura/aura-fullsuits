...
## Phase 8V-P7 additions (additive; resolves item 16's Scenario 6/7 half and part of the artifact question)

| # | Item | Severity | Status |
|---|---|---|---|
| 18 | Both Windows executables (`dist/AuraRetail.exe`, `dist/AuraClinic.exe`) predated the Phase 8V-P6 fix by 4 real days -- a real artifact-staleness gap Phase 8V-P6's own `build-impact-decision.md` did not catch | Found and fixed this session | **RESOLVED** -- both rebuilt from final HEAD, confirmed via PyInstaller's own change-detection log line. See `docs/owner/phase8vp7/artifact-dependency-analysis.md` |
| 19 | Item 16's Scenario 6/7 half | Blocks final tag | **RESOLVED for Owner-side mechanics** -- both scenarios reached real PASS using genuine multi-instance Windows product identities on one machine (corrected an earlier overly pessimistic "no VM/Sandbox" disclosure). Device-facing confirmation for Scenario 7 remains open. See `docs/owner/phase8vp7/scenario6-device-replacement-final.md` / `scenario7-plan-downgrade-overage-final.md` |
| 20 | Item 17 (stale-assertion, backup/restore/export, Clinic invoice/payment) | Blocks final tag | Still open after Phase 8V-P7 -- an extended real physical Android device disconnection partway through the session blocked this work after Scenario 2/3 completed. See `docs/owner/phase8vp7/phase8-final-unconditional-decision.md` |
| 21 | Scenario 2 (Retail late renewal) | Blocks final tag | **PARTIAL after Phase 8V-P7** -- restriction proven physically for Retail for the first time; renewal-restoration/88.00/return confirmation blocked by the same device disconnection |
