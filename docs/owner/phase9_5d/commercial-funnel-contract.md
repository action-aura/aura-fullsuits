...
| Record customer decision | `SENT` (all required approvals resolved APPROVED) | `ACCEPTED`/`REJECTED` | `quotes.create` (own) or `quotes.approve` (any) — Milestone 16 correction: `quotes.record_customer_decision` was never a real seeded permission, this recording action shares Create/Edit/Submit/Cancel's own-quote gate | ownership check | n/a | required on reject, recommended on accept | required | `QUOTE_CUSTOMER_DECISION` | `QUOTE_ACCEPTED`/`QUOTE_REJECTED_BY_CUSTOMER` | no (terminal for REJECTED; ACCEPTED proceeds) | **on ACCEPTED with a Lead-based Quote**: triggers Milestone 7's Lead-to-Customer conversion boundary before any Order may be created | acceptance alone creates no Payment/Subscription/License/Commission (Non-Negotiable 1) |
