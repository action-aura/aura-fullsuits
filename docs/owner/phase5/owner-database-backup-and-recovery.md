...
## A second round of fixes from automated commit review
- **Credential exposure via argv**: the initial implementation passed the full `postgresql://user:password@host/db` connection string as a `pg_dump`/`pg_restore` command-line argument -- visible to any other local user/process via `ps`/Task Manager. Fixed: connection is now passed as `--host`/`--port`/`--username`/`--dbname` flags (no secret), with the password supplied only via the `PGPASSWORD` environment variable of that one subprocess call. Verified: `owner/tests/test_backup_restore.py::test_password_never_appears_in_subprocess_argv` spies on the real `subprocess.run` call and asserts the password string never appears in `cmd`, only in `env["PGPASSWORD"]`.
- **Credential exposure via error message**: `pg_dump`/`pg_restore` stderr is stored in `error_detail` and surfaced in the audit log; some libpq error paths can echo connection details back. Fixed: `_redact_credential_leakage()` strips any `password=...` fragment before it is ever stored or audited, applied to every error path (subprocess exception, non-zero exit, restore failure).
- **Authorization-gate parity**: `restore_backup_route` hard-checked `actor.is_super_admin` in code (not just the `system.restore` permission grant); `create_backup_route` did not have the equivalent check. Fixed for parity. Verified: `owner/tests/test_security.py::test_backup_creation_hard_requires_super_admin_flag_not_just_permission`.
