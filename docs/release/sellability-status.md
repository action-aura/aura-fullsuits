...
| A second device can join the shop without inventing an admin | **RUN — backend proven on a fresh till, 2026-09-06; the screen is the remaining half** | The placeholder-admin leftover recorded under blocker 4 was a missing door plus one missing row: a device with no admin has no company id, so the sync parked the owner's account. The identity rebind now seeds `company_settings` with the licence id at activation when no admin exists (`company_rebind.seed_company_settings_for_joining_device`, five tests, mutation-proved). Proven on a fourth, empty till: activated by key with no session, restarted, and on the first status read after boot it already held the shop's four accounts; the desktop's owner signed in there as `ADMIN-0001` (`scripts/ops/join_e2e.py`). Design and the client-side door: `docs/launch-readiness/join-existing-shop-design.md` |
