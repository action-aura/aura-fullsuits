...
| Five themes on the phone, with a picker | **BUILT 2026-09-07; 325 unit tests, parity per theme against the desktop's own CSS; not yet on the handset (unplugged)** | Palettes behind the existing token names (getters over `AuraPalette.current`, so ~150 call sites recompose unchanged); Settings → Theme, persisted in `aura_prefs`; `DesktopTokenParityContractTest` compares each palette to its desktop block and goes red on a one-digit drift |
