...
| `EXTRA_DEVICE` add-on is sellable | **AVAILABLE, 50 JOD (data, 2026-09-05)** | Was `PLANNED` and unpriced. Set through the audited catalogue services on the rehearsal Owner per the owner's price list; device slots were already enforced. `EXTRA_BRANCH` followed on 2026-09-06 once the branch limit was enforced (see the owner's-asks table) |
| Licence activation end to end, on a clean machine | **RUN on a clean DATA DIR, 2026-09-06; the packaged exe on another machine is still unverified** | Three fresh tills (empty data directories, no relay URL configured, the rehearsal Owner on `:5551`) activated by key: one through the API (`scripts/ops/join_e2e.py`), two through the real first-run screen in Chromium (`scripts/ops/join_door_e2e.py`) — `SUCCESS`, `ACTIVE_ONLINE`, an Owner-issued `installation_id`, the relay address learned and used after restart, and a device slot consumed each time (allowance raised 3→6 through the audited op). What that does NOT cover: the PyInstaller/Inno build on a machine without the dev environment, which is what `go-live-runbook.md` Part 2 rehearses |
...
| A second device can join the shop without inventing an admin | **RUN — WORKS on the desktop, 2026-09-06 (backend + the real screen in Chromium); the Android door is built the same day (303 unit tests, decision mutation-proved) and PROVEN on the wiped Mi Note 10 the same evening — key, "Yes — connect to my shop", "Connecting to your shop…", sign-in, dashboard as `ADMIN-0001`; no admin created (`scripts/ops/phone_join_door.py`)** | The placeholder-admin leftover recorded under blocker 4 was a missing door plus one missing row: a device with no admin has no company id, so the sync parked the owner's account. The identity rebind now seeds `company_settings` with the licence id at activation when no admin exists (`company_rebind.seed_company_settings_for_joining_device`, five tests, mutation-proved). Proven on a fourth, empty till by API (`scripts/ops/join_e2e.py`), then through the REAL first-run screen in a real browser on a fifth and a sixth till (`scripts/ops/join_door_e2e.py`): "Already have a shop? Join it with your licence key" → key → "Connected to your shop" → restart → the desktop's owner signs in there through the ordinary form; the page made zero `create-admin` calls. The browser run caught a defect the unit tests had passed over — the boot path opened the setup modal a second time — fixed and pinned by a test that drives `init()` itself. Design, measurements, and what the Android door still needs: `docs/launch-readiness/join-existing-shop-design.md` |
| Inviting staff on a device whose staff list has synced | **WAS BROKEN — 500 on the demo laptop AND the phone, found 2026-09-06 17:52, FIXED and RUN the same hour** | `create_employee` minted `EMP-{COUNT(users)+1}`; five synced accounts numbered up to `EMP-0006` made count+1 = 6, and the insert died on `UNIQUE(company_id, employee_id)`. Now `max(count, highest suffix)+1` (`_next_employee_number`), byte-for-byte the old answer on any single-device install. Measured after: laptop invite → 200 with a setup link; a cashier created ON THE PHONE set a password, reached the laptop as active in 15 s and signed in there as `EMP-0008`; `docs/corrections/identity/employee-code-collision-after-sync-root-cause-analysis.md` |
| The invite link a new employee receives actually lets them set a password | **WAS A DEAD END — the link opened the plain sign-in screen; FIXED and RUN end to end 2026-09-06 20:40** | The Employees dialog issued `#setup/<token>` links that `init()` never handled (only `#verify-email/` and `#reset-password/`); the backend route existed and every earlier rehearsal had set the password over the API, so the runbook's step 2 read as proven. Now a "Set your password" page mirrors the reset-password screen; walked for real: dialog → link in a fresh browser → password set → the cashier signed in on the phone through its form. `retail_employee_setup_link_test.js` (6 checks, mutation-proved); `docs/corrections/identity/employee-setup-link-dead-end-root-cause-analysis.md` |
| The phone's dashboard tells a cashier the truth | **WAS WRONG — "TODAY'S SALES JD 0.000" minutes after a JD 18.000 sale; FIXED and RUN on the handset 2026-09-06 19:30 ("Till / Ready to sell" + the sentence, no figure); the owner's own phone dashboard still shows JD 55.035 / 3 transactions, matching the laptop** | `GET /dashboard/stats` answers 403 for a cashier by design (`retail.reports`); `DashboardScreen.kt` rendered `RetailStats()`'s zero defaults over the refusal. Now it does not fetch, and shows the desktop's own "Ready to sell" card and sentence; a failed fetch shows "Figures unavailable", never zeros. Found by walking the cashier's screens on the Mi Note 10 after the UI-driven sale; `DashboardWiringContractTest` pins it |
| The phone offers a cashier only what the server allows | **FIXED and RUN on the handset 2026-09-06 19:30 (no "Add Product" on the cashier's Products screen)** | "Add Product" (FAB and empty-state CTA) was shown to a cashier whose `POST /products` is refused (`CAP_STOCK_ADJUST`). Gated on `RetailSession.hasCapability(CAP_STOCK_ADJUST)`; `ProductsWiringContractTest` pins the string against the server's |
| Products by spreadsheet, through the real Import wizard | **RUN 2026-09-07 00:30 on the laptop; the rows reached the phone** | Products → ⬆ Import → a three-row CSV in the template's own column names (Pepsi 330ml, Chips Salt 45g, Water 1.5L; barcodes, categories, cost/sell, tax, unit, reorder level, opening stock) → every column auto-mapped on step 2 → step 3 "3 clean records, 0 removed, 12 type coercions" → "Import Complete: 3 new products, 0 updated" → rows in the list, and on the phone's product list on its next pull. Kept in the catalogue as the "products for Ahmed" demo rows |
| A barcode "scan" lands in the laptop's POS cart | **RUN 2026-09-07 00:50 — a keystroke burst, the way a HID scanner types** | With no field focused on the POS, `6281000000011` typed in a burst + Enter: toast "Added: Pepsi 330ml", one cart line at JD 0.500, tax JD 0.080, total JD 0.580, status "Ready to scan". Category chips (Drinks / Snacks) appeared from the imported categories. Characterised on the Scanner settings page ("Start Test", then a burst): with the default 50 ms window, three codes at 5 ms and 15 ms per key captured intact, at 30/60/120 ms nothing (the buffer resets between keys, by design; the window is a setting, 10-500 ms, and "keyboard" mode triples it). The POS resolves through the exact-code lookup route, not the substring search |
| The laptop's Reports screen, as the owner | **RUN 2026-09-07 00:50** | Last 14 days: revenue JD 245.725, 6 transactions, gross profit JD 125.725 (51.2%), avg ticket JD 40.954; sales by employee (a since-removed account shown honestly as "Account removed"), daily revenue trend, payment-method donut. Fils intact throughout |
| Every step of the demo runbook, through the real screens | **RUN 2026-09-06 evening, both devices, no API shortcuts** | Laptop Employees dialog → invite link in a fresh browser → "Set your password" page → that cashier signed in on the phone through its form → laptop "Add Product" dialog (Demo Water 500ml, JD 0.750, stock 10) → on the phone's Products tab with "10 in stock" → a sale rung through the phone's POS → on the laptop's recent transactions within a second. Drivers: `join-door-shots/rehearsal/*.png`, `scripts/ops/phone_ui.py`, the Playwright scripts under the job tmp |
| A sale rung through the phone's real POS screen | **RUN — WORKS, 2026-09-06 18:41** | Cashier signed in through the form → POS → cake → View cart → Charge → "Payment successful, JD 18.000 collected, SALE-000003"; on the laptop's recent transactions in under a second, cake stock 71→70 both sides (`scripts/ops/phone_ui.py`) |
| The phone in Arabic | **RUN 2026-09-06 evening — layout mirrors, every Settings/More label is Arabic; one gap** | Switched through Settings → Language on the handset: right-to-left takes effect at once (bottom bar and Back mirror too). Seeded payment-method NAMES stay English ("Cash / Card / Bank Transfer…") because they are `payment_methods` rows, not `tr()` keys — ROADMAP item; the desktop has the same rows |
| The demo laptop is the shop's admin device | **RUN — claimed 2026-09-06 19:05 through the real banner** | After the tills were restarted `/api/devices/me` said `can_claim_admin: true` and Settings/Audit Log/Employees/Branches were hidden; one click on "Make this the admin device" restored them. Pre-flight step added to `sunday-demo-runbook.md` §0 |
| Five themes on the laptop, night mode back | **RUN 2026-09-07 02:40 — all five applied on the running till and screenshotted** | Day (the light), Sand (warm paper), Calm (the dark the phone had), Night (ink + aurora teal), Dusk (violet + lavender). Each is one token block between markers; the safety test holds every theme to the dark theme's structure and the contrast test runs its palette cross-products and the full rendered-corpus sweep per theme (43 checks). The real page caught what node could not: a global-name collision between the boot script and the shell that blanked the app, fixed and pinned. Picker: the 🎨 button; persisted under `aura_theme_v2` |
| Five themes on the phone, with a picker | **RUN 2026-09-07 19:08–19:10 — all five applied through the real picker on the Mi Note 10 and screenshotted** (`scripts/ops/phone_theme_check.py`; Day/Sand/Night/Dusk/Calm each render their own ground, accent on prices, green stock pills, tinted nav highlight; no defect; phone left on Calm at sign-in) | Palettes behind the existing token names (getters over `AuraPalette.current`, so ~150 call sites recompose unchanged); Settings → Theme, persisted in `aura_prefs`; `DesktopTokenParityContractTest` compares each palette to its desktop block and goes red on a one-digit drift |
| A brand: mark, app icon, wordmark, intro | **FIRST CUT 2026-09-07, committed under `products/retail/frontend/brand/`, not yet wired into the apps** | Open ring with a spark around an ink A; Outfit wordmark; six-second self-contained intro; showcase artifact "Aura Brand" |
| Deletions converge | **RUN — WORKS, 2026-09-06** | Three products and two customers deleted on till A through the real DELETE routes (soft-deletes) were gone from till B's lists after 5 s (`scripts/ops/cleanup_test_rows.py`). Done to clean the rehearsal data; recorded because it is the first time the delete half of the sync was run on two real installs |
| A branch can be renamed, and the rename converges | **RUN — WORKS, 2026-09-06** | Found writing the demo runbook: `/branches` had only GET and POST, so a mistyped branch name lived forever and synced everywhere. Added `PUT /branches/<id>` (name/address/phone; tenant-scoped; emits the existing `branch` update event, whose apply path already upserted by uid) and an Edit control on the desktop Branches screen. Proven on two tills: renamed on A, B showed the new name after 10 s (`scripts/ops/branch_rename_e2e.py`). Six route tests + JS screen tests with mutation proofs; dropping the sync emission turned two red. Retiring a branch remains its own design |
...
| A new **branch 250 JOD** | **ENFORCED and SELLABLE — RUN on the real till, 2026-09-06** | Built the same night with no schema change on either side: a `max_branches` integer entitlement (seeded definition; the rehearsal plan sets 1, the `EXTRA_BRANCH` add-on sets 2, a per-licence override covers more) rides the signed assertion like every other entitlement, and the till's `create_branch` refuses past it with `403 BRANCH_LIMIT` and a catalogued sentence — absent or 0 means no limit, so no existing shop is ever locked out by the seed. Proven on the desktop till (`scripts/ops/branch_limit_e2e.py`): a forced check-in delivered `max_branches: 1`, the second branch was refused; then the add-on was attached to the subscription and made AVAILABLE (`scripts/ops/owner_rehearsal_buy_branch.py`), the next check-in delivered 2, and the same till created branch 2. Six retail tests; disabling the gate turned two red. `EXTRA_BRANCH` is now **AVAILABLE, 250 JOD** on the rehearsal Owner |
...
4. ~~**Accounts do not sync to the phone.**~~ **CLOSED 2026-09-05 — RUN on two
   ...
   **And on the phone, 2026-09-06 00:38:** the APK rebuilt with this change
   was installed over the running app (data kept) by `scripts/ops/
   phone_watcher.ps1` the moment the handset was plugged in; the desktop's
   owner then logged in on the phone's own backend (`200`, shown there as
   `ADMIN-0003`), and the shop currency changed on the desktop read back on
   the phone within 10 s in both directions (`scripts/ops/
   two_till_settings.py`'s phone variant). One owner login, one set of shop
   settings, on the real phone.
