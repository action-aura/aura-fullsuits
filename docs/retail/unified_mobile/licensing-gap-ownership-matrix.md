...
| 2 | No customer-facing login/portal identity | `OWNER_SERVER` | `customer-authentication-gap-analysis.md` | Would require new `Customer`-scoped auth model, session table, and routes in `owner/app/`. |
...
| 6 | `commercial_runtime/licensing_contracts/canonical.py`'s own docstring references a shared fixture file (`tests/fixtures/canonical_vectors.json`) that does not exist | `COMMERCIAL_RUNTIME` | `offline-license-lease-contract-audit.md` | Owned by that package's own test suite/maintainers; M7 works around it with an independent mobile-side fixture set rather than fabricating the missing upstream file. |
