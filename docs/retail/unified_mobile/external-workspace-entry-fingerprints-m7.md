# External Workspace Entry Fingerprints (M7 Entry)

Real, executed fingerprints captured at the exact start of M7 (Unified
Mobile branch HEAD `4e1993fee920d81bb00cc9c5e5b1bb415d37685b` at
...
```
HEAD: 414e6ea5ca9fc698fe075a4ae4464cebf0b7bf34   (unchanged since M6 entry/exit)

STATUS (git status --short): 22 modified, 19 untracked (identical file list to M6's captures)
...
Every value is byte-identical to `external-workspace-exit-fingerprints-m6.md`'s
...
```
HEAD: 8c35768393d9407f72ef34aa93705f9c454453f7   (unchanged since M6 entry/exit)
...
Identical to M6's own exit capture — no real Phase 9R work has landed
between the two milestones' fingerprint checks.

## What "unchanged" will mean at M7 exit

At M7 exit, the same 8 real commands will be re-run against both
paths. Required conclusion:
`UNCHANGED_RELATIVE_TO_M7_ENTRY_STATE` — every value above must match
