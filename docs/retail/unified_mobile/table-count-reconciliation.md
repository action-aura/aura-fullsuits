...
## Addendum (M5.5 follow-up) — 21st table, real and deliberate, no legacy source

`import_conflicts` (`Catalog.sq`) was added after M5.5 to give `CatalogImporter`'s deterministic barcode/SKU dedup pass a real, durable audit trail (product-migration-audit-report.md) — the governing spec's own explicit requirement that a dropped/skipped value be recorded, not silently discarded. **This is the one genuine exception to "zero net-new table shapes beyond a faithful port"** stated above: `import_conflicts` has no legacy counterpart at all, because the legacy system never needed one (it never enforced the uniqueness that could produce a conflict in the first place). The unified schema is now **21 tables**, not 20 — this addendum exists specifically so a later reader does not have to rediscover that fact from the `.sq` files themselves. `inventory_movements`' own earlier column additions (`inventory-movement-authority-decision.md`) remain the only other post-M5.0 schema change, and that one added no new table, only columns.
