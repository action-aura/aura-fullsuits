...
class OwnerClientTest {
    ...
    fun `signature differs across two activations from the same key due to fresh nonce`() {
        ...
        server.enqueue(MockResponse().setResponseCode(200).setBody("""{"result":"SUCCESS"}"""))
        val c = client()
        c.activate("AURA_CLINIC", "ANDROID", "1.0.0-rc.2", "rc", "id-1", identity.getPublicKeyB64(), "KEY-A", "idem-1", identity)
        c.activate("AURA_CLINIC", "ANDROID", "1.0.0-rc.2", "rc", "id-1", identity.getPublicKeyB64(), "KEY-A", "idem-2", identity)
