...
class ProductUseCasesTest {
    ...
    private fun createUseCase(db: RetailDatabase) =
        CreateProductUseCase(SqlDelightProductRepository(db), SqlDelightCategoryRepository(db), normalizer)

    @Test
    fun createValidProductSucceeds() = runTest {
        val db = newDb()
        val result = createUseCase(db).execute(1L, "SKU-001", "0000000001", "Cola 330ml", null, Money.of(0.5), Money.of(1.99), PercentageRate.trusted(10.0), "can", 24, 1000L)
    ...
    fun createAcceptsArabicNameAndPreservesItExactly() = runTest {
        val db = newDb()
        val result = createUseCase(db).execute(1L, "SKU-AR", null, "مشروب غازي", null, Money.ZERO, Money.of(2.0), PercentageRate.trusted(0.0), "can", 5, 1000L)
    ...
    fun createTrimsLeadingAndTrailingWhitespaceFromName() = runTest {
        val db = newDb()
        val result = createUseCase(db).execute(1L, "SKU-002", null, "  Cola 330ml  ", null, Money.ZERO, Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 1000L)
        assertIs<DomainResult.Success<com.actionaura.retail.data.model.Product>>(result)
    ...
    fun createRejectsBlankName() = runTest {
        val db = newDb()
        val result = createUseCase(db).execute(1L, "SKU-003", null, "   ", null, Money.ZERO, Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 1000L)
    ...
    fun createRejectsNegativeCostAndSellPrice() = runTest {
        val db = newDb()
        val negCost = createUseCase(db).execute(1L, "SKU-004", null, "Widget", null, Money.of(-1.0), Money.of(1.0), PercentageRate.trusted(0.0), "pcs", 5, 1000L)
        assertIs<DomainResult.Failure>(negCost)
        assertIs<RepositoryError.ValidationFailed>(negCost.error)

        val negSell = createUseCase(db).execute(1L, "SKU-005", null, "Widget", null, Money.of(1.0), Money.of(-1.0), PercentageRate.trusted(0.0), "pcs", 5, 1000L)
    ...
    fun createWithActiveCategorySucceeds() = runTest {
        val db = newDb()
        val category = (CreateCategoryUseCase(SqlDelightCategoryRepository(db), normalizer).execute(1L, "Beverages", null, 500L) as DomainResult.Success).value
        val result = createUseCase(db).execute(1L, "SKU-006", null, "Cola", category.id, Money.ZERO, Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 1000L)
        assertIs<DomainResult.Success<*>>(result)
    }
    ...
    fun createRejectsArchivedCategoryAssignment() = runTest {
        ...
        val result = createUseCase(db).execute(1L, "SKU-007", null, "Cola", category.id, Money.ZERO, Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 1000L)
        assertIs<DomainResult.Failure>(result)
        assertIs<RepositoryError.ValidationFailed>(result.error)
    }
    ...
    fun createRejectsCrossBusinessCategoryAssignment() = runTest {
        ...
        val result = createUseCase(db).execute(2L, "SKU-008", null, "Cola", categoryUnderCompany1.id, Money.ZERO, Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 1000L)
        assertIs<DomainResult.Failure>(result)
    ...
    fun createRejectsDuplicateSkuCaseInsensitive() = runTest {
        val db = newDb()
        createUseCase(db).execute(1L, "SKU-009", null, "Cola", null, Money.ZERO, Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 1000L)
        val result = createUseCase(db).execute(1L, "sku-009", null, "Cola 2", null, Money.ZERO, Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 2000L)
        assertIs<DomainResult.Failure>(result)
    ...
    fun createAllowsSameSkuInDifferentCompanies() = runTest {
        val db = newDb()
        assertIs<DomainResult.Success<*>>(createUseCase(db).execute(1L, "SKU-010", null, "Cola", null, Money.ZERO, Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 1000L))
        assertIs<DomainResult.Success<*>>(createUseCase(db).execute(2L, "SKU-010", null, "Cola", null, Money.ZERO, Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 1000L))
    }

    @Test
    fun createRejectsDuplicateBarcode() = runTest {
        val db = newDb()
        createUseCase(db).execute(1L, "SKU-011", "000123", "Cola", null, Money.ZERO, Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 1000L)
        val result = createUseCase(db).execute(1L, "SKU-012", "000123", "Sprite", null, Money.ZERO, Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 2000L)
        assertIs<DomainResult.Failure>(result)
        assertIs<RepositoryError.DuplicateValue>(result.error)
    ...
    fun createPreservesLeadingZeroBarcodeExactly() = runTest {
        val db = newDb()
        val result = createUseCase(db).execute(1L, "SKU-013", "0000012345", "Cola", null, Money.ZERO, Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 1000L)
        assertIs<DomainResult.Success<com.actionaura.retail.data.model.Product>>(result)
    ...
    fun createPreservesLongBarcode() = runTest {
        ...
        val result = createUseCase(db).execute(1L, "SKU-014", longBarcode, "Cola", null, Money.ZERO, Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 1000L)
        assertIs<DomainResult.Success<com.actionaura.retail.data.model.Product>>(result)
    ...
    fun createAllowsNullBarcodeAndMultipleNullsDoNotConflict() = runTest {
        val db = newDb()
        assertIs<DomainResult.Success<*>>(createUseCase(db).execute(1L, "SKU-015", null, "Cola", null, Money.ZERO, Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 1000L))
        assertIs<DomainResult.Success<*>>(createUseCase(db).execute(1L, "SKU-016", null, "Sprite", null, Money.ZERO, Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 1000L))
    }

    @Test
    fun createRejectsBlankButPresentBarcode() = runTest {
        val db = newDb()
        val result = createUseCase(db).execute(1L, "SKU-017", "   ", "Cola", null, Money.ZERO, Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 1000L)
        assertIs<DomainResult.Failure>(result)
        assertIs<RepositoryError.ValidationFailed>(result.error)
    }
    ...
    fun skuCannotBeReusedAfterArchiving() = runTest {
        ...
        val product = (createUseCase(db).execute(1L, "SKU-025", null, "Cola", null, Money.ZERO, Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 1000L) as DomainResult.Success).value
        ArchiveProductUseCase(productRepo).execute(1L, product.id, 2000L)

        val result = createUseCase(db).execute(1L, "SKU-025", null, "Cola 2", null, Money.ZERO, Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 3000L)
        assertIs<DomainResult.Failure>(result)
        assertIs<RepositoryError.DuplicateValue>(result.error)
        assertEquals("sku", result.error.field)
    }
    ...
    fun archiveThenReactivateRoundTrip() = runTest {
        val db = newDb()
        val productRepo = SqlDelightProductRepository(db)
        val product = (createUseCase(db).execute(1L, "SKU-018", null, "Cola", null, Money.ZERO, Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 1000L) as DomainResult.Success).value
    ...
    fun archivedProductCannotReceiveNewCategoryAssignmentButKeepsExistingOne() = runTest {
        ...
        val productRepo = SqlDelightProductRepository(db)
        val category = (CreateCategoryUseCase(categoryRepo, normalizer).execute(1L, "Beverages", null, 500L) as DomainResult.Success).value
        val product = (createUseCase(db).execute(1L, "SKU-019", null, "Cola", category.id, Money.ZERO, Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 1000L) as DomainResult.Success).value
    ...
    fun staleUpdateIsRejected() = runTest {
        ...
        val product = (createUseCase(db).execute(1L, "SKU-020", null, "Cola", null, Money.of(0.5), Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 1000L) as DomainResult.Success).value
    ...
    fun updateDoesNotAllowChangingBarcodeToAnotherProductsBarcode() = runTest {
        ...
        val productRepo = SqlDelightProductRepository(db)
        val categoryRepo = SqlDelightCategoryRepository(db)
        createUseCase(db).execute(1L, "SKU-021", "000999", "Sprite", null, Money.ZERO, Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 1000L)
        val target = (createUseCase(db).execute(1L, "SKU-022", null, "Cola", null, Money.ZERO, Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 1000L) as DomainResult.Success).value
    ...
    fun searchFindsByNormalizedPrefixIncludingFullWidthVariant() = runTest {
        val db = newDb()
        createUseCase(db).execute(1L, "SKU-023", null, "Cola 330ml", null, Money.ZERO, Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 1000L)
        val search = SearchProductsUseCase(SqlDelightProductRepository(db), normalizer)
    ...
    fun findByBarcodeOnlyResolvesActiveProducts() = runTest {
        val db = newDb()
        val productRepo = SqlDelightProductRepository(db)
        val product = (createUseCase(db).execute(1L, "SKU-024", "000555", "Cola", null, Money.ZERO, Money.of(1.0), PercentageRate.trusted(0.0), "can", 5, 1000L) as DomainResult.Success).value
