...
def build_subscription_timeline(subscription_id) -> list[TimelineEvent]:
    ...
    for h in db_session.execute(
        ...
        from_label = subscription_status_label(h.from_status) if h.from_status else _("created")
        events.append(TimelineEvent(h.created_at, "SUBSCRIPTION", _("Subscription %(from_status)s -> %(to_status)s", from_status=from_label, to_status=subscription_status_label(h.to_status)), "subscription", str(h.subscription_id), h.reason))
    ...
    if license_ids:
        for h in db_session.execute(select(LicenseStatusHistory).where(LicenseStatusHistory.license_id.in_(license_ids))).scalars().all():
            from_label = license_status_label(h.from_status) if h.from_status else _("created")
            events.append(TimelineEvent(h.created_at, "LICENSE", _("License %(from_status)s -> %(to_status)s", from_status=from_label, to_status=license_status_label(h.to_status)), "license", str(h.license_id), h.reason))
    ...
    if installation_ids:
        for h in db_session.execute(select(InstallationStatusHistory).where(InstallationStatusHistory.installation_id.in_(installation_ids))).scalars().all():
            from_label = installation_status_label(h.from_status) if h.from_status else _("registered")
            events.append(TimelineEvent(h.created_at, "INSTALLATION", _("Installation %(from_status)s -> %(to_status)s", from_status=from_label, to_status=installation_status_label(h.to_status)), "installation", str(h.installation_id), h.reason))
    ...
    if renewal_ids:
        for h in db_session.execute(select(RenewalRequestStatusHistory).where(RenewalRequestStatusHistory.renewal_request_id.in_(renewal_ids))).scalars().all():
            from_label = renewal_status_label(h.from_status) if h.from_status else _("created")
            events.append(TimelineEvent(h.created_at, "RENEWAL", _("Renewal %(from_status)s -> %(to_status)s", from_status=from_label, to_status=renewal_status_label(h.to_status)), "renewal_request", str(h.renewal_request_id), h.reason))
    ...
    if pilot_ids:
        for h in db_session.execute(select(PilotStatusHistory).where(PilotStatusHistory.pilot_record_id.in_(pilot_ids))).scalars().all():
            from_label = pilot_status_label(h.from_status) if h.from_status else _("created")
            events.append(TimelineEvent(h.created_at, "PILOT", _("Pilot %(from_status)s -> %(to_status)s", from_status=from_label, to_status=pilot_status_label(h.to_status)), "pilot_record", str(h.pilot_record_id), h.reason))
