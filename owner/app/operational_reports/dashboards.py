...
def management_operational_dashboard(currency: str) -> dict:
    ...
    expense_totals_by_category = dict(db_session.execute(
        ...
        .where(Expense.currency == currency, Expense.status.notin_(("VOID",)))
    ...
    approval_queue_count = db_session.execute(
        select(func.count(ExpenseApproval.id)).where(ExpenseApproval.status == "PENDING")
    ).scalar_one()
    ...
    duplicate_review_queue = db_session.execute(
        select(func.count(func.distinct(Expense.external_reference)))
        .where(Expense.external_reference.is_not(None), Expense.status.notin_(("VOID", "REJECTED")))
        .group_by(Expense.external_reference)
        .having(func.count(Expense.id) > 1)
    ).all()
    duplicate_review_queue_count = len(duplicate_review_queue)
