/**
 * clinic_einvoicing_status_test.js — the e-invoicing status card's "which
 * provider is actually running" fix, ported from
 * products/retail/tests/retail_einvoicing_status_test.js onto
 * products/clinic/frontend/einvoicing.js.
 *
 ...
 * becoming ON by default in Jordan, the common case for a fresh clinic is
 ...
 * Provider: mock, Queued: 47" in that state tells a clinic nothing they
 ...
 *      Unlike retail, the clinic file has no label for ordinary provider
 *      names (only 'unconfigured' -> 'Not connected'), so this renders the
 *      raw string, matching the clinic file's own pre-existing behaviour.
 ...
 *
 * Run: node products/clinic/tests/clinic_einvoicing_status_test.js
 */
'use strict';
...
// ─────────────────────────────────────────────────────────────────────────────
// Mutation harness (same shape as retail_einvoicing_status_test.js)
// ─────────────────────────────────────────────────────────────────────────────
...
// as retail_einvoicing_status_test.js), handler(callCount) -> body
// ─────────────────────────────────────────────────────────────────────────────
...
async function notConnectedShowsNoticeAndDisablesButton(scriptSrc) {
  ...
  assert.ok(/JoFotara portal/i.test(notice.textContent) && /Client-ID/i.test(notice.textContent),
    'Notice text did not tell the clinic how to connect (portal + Client-ID/Secret-Key/activity number).');
  assert.ok(/invoice/i.test(notice.textContent) && !/\bsale\b/i.test(notice.textContent),
    `Notice text should say "invoice", not "sale", for the clinic product: ${JSON.stringify(notice.textContent)}`);
...
//     value. The clinic file (unlike retail) has no PROVIDER_LABELS entry
//     for ordinary provider names, so this must render the raw string --
//     that is the clinic file's own pre-existing behaviour, unchanged.
// ─────────────────────────────────────────────────────────────────────────────
...
async function testBackwardCompatibleWithOlderBackend() {
  ...
  const providerRow = findProviderRow(statusContent);
  assert.strictEqual(providerRow.textContent, 'mock',
    `Expected the old 'provider' value ("mock") to still render as-is, got: ${JSON.stringify(providerRow.textContent)}`);
...
async function main() {
  ...
  if (failures.length) {
    console.error(`\nFAIL: clinic_einvoicing_status_test.js — ${failures.length} of ${CHECKS.length} checks failed:`);
    for (const name of failures) console.error(`  - ${name}`);
    process.exitCode = 1;
    return;
  }
  console.log(`PASS: clinic_einvoicing_status_test.js — ${CHECKS.length} checks`);
}

main().catch((err) => {
  console.error('FAIL: clinic_einvoicing_status_test.js (runner)');
  console.error(err);
  process.exitCode = 1;
});
