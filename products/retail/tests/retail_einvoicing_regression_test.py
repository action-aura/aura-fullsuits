...
CUSTOMERS_TABLE_COLUMNS = [
    ...
    'total_spent', 'created_at', 'credit_mode', 'credit_limit', 'credit_balance',
]
...
def _make_admin_and_product(price=100.0, tax_rate=10.0, stock=50):
    ...
    rconn.execute(
        "INSERT INTO products (company_id,sku,name,cost_price,sell_price,tax_rate) VALUES (?, 'ER-1','Regression Item',5,?,?)",
        (company_id, price, tax_rate),
    )
    product_id = rconn.execute(
        "SELECT id FROM products WHERE company_id=? AND sku='ER-1'", (company_id,)
    ).fetchone()[0]
    rconn.execute(
